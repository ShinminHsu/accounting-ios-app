...
The system SHALL allow the user to upload a credit card bill as a PDF file or image screenshot. The system SHALL send the file to Claude API and receive a structured list of line items, each containing: date (YYYY-MM-DD), merchant name, and amount in the card's currency.
